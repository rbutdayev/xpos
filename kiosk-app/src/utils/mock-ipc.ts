...
export const mockIPC: IPCApi = {
  ...
  async uploadLogo() {
    // Mock implementation - return null (no file selected)
    return null;
  },

  // Authentication
  async loginWithPin(userId: number, pin: string) {
    // Mock implementation
    return {
      user_id: userId,
      user_name: `User ${userId}`,
      branch_id: mockConfig?.branch_id || 1,
    };
  },

  async verifyKioskPin(params: {user_id: number; pin: string}) {
    // Mock implementation - accept any PIN for testing
    return {
      success: params.pin.length >= 4,
    };
  },
  ...
  async logout() {
    // Mock implementation
    if (mockConfig) {
      mockConfig.user_id = undefined;
      mockConfig.user_name = undefined;
      mockConfig.is_logged_in = false;
    }
  },
