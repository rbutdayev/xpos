...
export default function Setup() {
  ...
  const { setConfig, setLoading, setError, loadTaxRate } = useConfigStore();
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // Wait for services to initialize (restartServices is async in main process)
      setSyncMessage('Initializing services...');
      await new Promise((resolve) => setTimeout(resolve, 2000)); // Wait 2 seconds for services to start
      setSyncProgress(60);

      // Initial sync
      setSyncMessage('Syncing products, customers, and users...');
      try {
        await window.ipc.triggerSync();
      } catch (error) {
        console.warn('Initial sync failed (will retry automatically):', error);
        // Don't fail registration if sync fails - it will retry automatically
      }
      setSyncProgress(100);
      ...
      // Load tax rate from fiscal config
      await loadTaxRate();
