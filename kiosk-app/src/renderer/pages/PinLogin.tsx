...
export default function PinLogin() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      toast.success(`${t('pinLogin.welcomeBack')}, ${userData.user_name}!`);
      ...
      // Extract clean error message from Electron IPC wrapper
      let errorMessage = error.message || t('pinLogin.loginFailed');

      // Remove "Error invoking remote method 'auth:loginWithPin': Error: " prefix
    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-[70%] relative overflow-hidden flex items-center justify-center">
        ...
        {!config?.logo_path && (
          <div className="relative z-10 text-center text-white px-12">
            ...
            <p className="text-2xl mb-8 text-blue-100">{t('app.title')}</p>
            ...
            <div className="space-y-4 max-w-md mx-auto">
              <div className="flex items-center space-x-3 text-left">
                ...
                <span className="text-lg">Offline-first POS System</span>
              </div>
              <div className="flex items-center space-x-3 text-left">
                <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 bg-white/20">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span className="text-lg">Secure & Fast</span>
              </div>
              <div className="flex items-center space-x-3 text-left">
                <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 bg-white/20">
                  <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <span className="text-lg">Auto Sync</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="w-[30%] bg-gray-50 flex items-center justify-center p-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('pinLogin.title')}</h1>
          <p className="text-gray-600">{t('pinLogin.subtitle')}</p>
        </div>
        ...
        <div className="mb-6">
          ...
          <input
            ...
            onChange={(e) => {
              // Prevent any onChange from happening (backup safety)
              e.preventDefault();
            }}
            onKeyDown={(e) => {
              // Block all keyboard input except Enter for login
              if (e.key === 'Enter') {
                handleLogin();
              } else if (e.key !== 'Tab') {
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((digit) => (
            <button
              ...
              disabled={isLoggingIn || (focusedField === 'pin' && pin.length >= 6)}
          ...
          <button
            onClick={handleClear}
            disabled={isLoggingIn || (focusedField === 'userId' ? userId.length === 0 : pin.length === 0)}
        ...
        <button
          onClick={handleLogin}
          disabled={!userId || pin.length < 4 || isLoggingIn}
        ...
        <div className="mt-4 text-center">
          <button
            onClick={async () => {
              if (confirm('Are you sure you want to reset device settings? You will need to re-register the device.')) {
                try {
                  await window.ipc.clearConfig();
                  toast.success('Settings cleared. Redirecting to setup...');
                  setTimeout(() => {
                    navigate('/setup');
                  }, 500);
                } catch (error) {
