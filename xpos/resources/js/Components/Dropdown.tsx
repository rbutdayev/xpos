...
const Content = ({
    ...
    return (
        <>
            ...
            >
                <div
                    className={`absolute z-[70] ${direction === 'up' ? 'mb-2 bottom-full' : 'mt-2'} rounded-md shadow-lg ${alignmentClasses} ${widthClasses}`}
                    onClick={() => setOpen(false)}
                >
                    <div
                        className={
...
const DropdownLink = ({
    ...
    return (
        <Link
            ...
            onClick={handleClick}
            className={
                'block w-full px-4 py-2 text-start text-sm leading-5 text-slate-200 transition duration-150 ease-in-out hover:bg-slate-600 focus:bg-slate-600 focus:outline-none ' +
                className
            }
        >
            {children}
